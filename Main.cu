...
//const unsigned int   L_Gap = 512; //=L_B 
const unsigned int   L_Gap = 64; //=L_B 
...
main( int argc, char** argv) 
{
	...
	int NumProcs, MyProc, NameLength, devCount;
	char ProcessorName[MPI_MAX_PROCESSOR_NAME];
	
	//Accept command line arguments
	int L_A1 = 0;
	int L_B = 0;
	if(sscanf( argv[1], "%d", &L_A1)!= 1 || sscanf(argv[2], "%d", &L_B)!=1)
	{
        printf("Provide arguemnts for main and test sequence lengths\n");
        return 0;
    }
	...
	CUDA_SAFE_CALL(cudaSetDevice(MyProc % devCount));

    //Code to wait for debugger to get attached
    if(MyProc==-1)
    {
        int i = 0;
        char hostname[256];
    ...
    float overallTimeKernel1_2Start = cutGetTimerValue(hTimer);
    ...
    float overallTimeKernel1_2End = cutGetTimerValue(hTimer);

    float overallTimeKernel1_2Max;
    float overallTimeKernel1_2 = overallTimeKernel1_2End - overallTimeKernel1_2Start;
    MPI_Reduce(&overallTimeKernel1_2, &overallTimeKernel1_2Max, 1, MPI_FLOAT, MPI_MAX, 0, MPI_COMM_WORLD);
    if(MyProc==0)
	    printf("Overall Application Time        :   %f ms \n", overallTimeKernel1_2Max);
