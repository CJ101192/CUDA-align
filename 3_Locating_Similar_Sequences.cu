								 ...
								 int K_3_R, int MyProc, int K3_Report, int K3_Safety, int NumProcs, int K3_Length, int K3R, int BlockSize_K3_X, int GridSize_K3_X)
{
	...
	dim3 GridSize_K3 (GridSize_K3_X, 1);        // 128
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	MPI_Barrier(MPI_COMM_WORLD);
   	float Start_Kernel_3_Comm  =	cutGetTimerValue(hTimer);
	...
	MPI_Gather( h_Length_Seq_K4   , DATA_SZ_K38/4, MPI_INT, h_Length_Seq_K4_MPI     , DATA_SZ_K38/4, MPI_INT, 0, MPI_COMM_WORLD);
   	float End_Kernel_3_Comm  =	cutGetTimerValue(hTimer);
	...
	if (MyProc==0)
	{
		...
		unsigned int similarityReport    = measurePeriodicSimilarity(Max_K3_K4_CPU, h_Max_K3_K4_All, K3_Report, K3_Report * K_3_R,K3_Report * K_3_R);
        //Tolerance threshold is set to 95%
		unsigned int result_regtest_K3   = similarityReport < (K3_Report * K_3_R * 0.95)? 0:1;
        printf("Kernel 3 Similarity             :   %f\n", (float)similarityReport/(K3_Report*K_3_R));
		printf("CPU execution time for Kernel 3 :   %f ms \n", End_Kernel_3_CPU -Start_Kernel_3_CPU);
        float gpuTotalTime =  (End_Kernel_3 -Start_Kernel_3)+(End_Kernel_3_4 -Start_Kernel_3_4);
		printf("GPU execution time for Kernel 3 :   %f ms \n", gpuTotalTime);
        printf("Kernel 3 Comm Ratio             :   %f \n",(End_Kernel_3_Comm - Start_Kernel_3_Comm)/gpuTotalTime);
