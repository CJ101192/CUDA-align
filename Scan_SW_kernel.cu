...
__device__ void clearLastElement(int* s_data, 
                                 int *g_blockSums,
                                 int* g_blockStride, 
                                 int blockIndex, unsigned int stride)
{
    if (threadIdx.x == 0)
    {
        ...
        if (storeSum) // compile-time decision
        {
            ...
            g_blockStride[blockIndex]=stride; 
        }
...
//------------------------------- Ali ----------------------------------
		...
		{
//			while (g_blockSums[blockIndex-1]==0 && g_blockStride[blockIndex-1]==0)
//				s_data[index]=0;
			s_data[index] = g_blockSums[blockIndex-1]-g_blockStride[blockIndex-1];
		}
//------------------------------- Ali ----------------------------------
//      s_data[index] = 0;
    }
...
///------------------------- Ali ------------------------
            s_data[bi] = max(s_data[ai],s_data[bi]+stride);
//            s_data[bi] += s_data[ai];
///------------------------- Ali ------------------------
        }
...
__device__ void scanRootToLeaves(int *s_data, unsigned int stride)
{
    ...
    for (int d = 1; d <= blockDim.x; d *= 2)
    {
        ...
        if (thid < d)
        {
            ...
            s_data[ai] = s_data[bi];
//---------------------- Ali --------------------------
            s_data[bi] = max(t,s_data[bi])-stride;
//---------------------- Ali --------------------------
//				s_data[bi] += t;
        }
    }
...
template <bool storeSum>
__device__ void prescanBlock(int *data, int blockIndex, int *blockSums, int* blockStride)
{
    int stride = buildSum(data);               // build the sum in place up the tree

    clearLastElement<storeSum>(data, blockSums, blockStride, 
                               (blockIndex == 0) ? blockIdx.x : blockIndex, stride);
                        ...
                        int baseIndex)
{
    ...
    prescanBlock<storeSum>(s_data, blockIndex, g_blockSums, g_blockStride); 
